-- Q # 1:Write a query to display student information such as name, branch in capital letters.
select upper(student_name), upper(branch) from student_info;

-- Q # 2:Write a query to displays all details in subject_master in small letters.
select lower(subject_code), lower(subject_name), lower(weightage) from subject_master;

-- Q # 3:Write two separate queries to display the registration number date of birth  of all the students in the following formats• 2011/07/23
...
-- Q # 4:Write a query to display age of each student along with name, contact number and email id. Age = Number of months between DOB and current date /12.
...
-- Q # 5:Write a query to display the registration number, student name and average marks secured by students in each semester.
...
-- Q # 6:Pick the maximum mark from the students_marks and display the student registration number and name of those students who have secured the maximum mark.
...
-- Q # 7:Pick the maximum marks secured in the subject “ EI05IP” and display the student name and registration number of the student who has secured it.
...
-- Q # 8:Write a query to display the average GPA for each semester. Display the semester number and the average. Hint: Average = Total GPA of all students in a semester/total number of students in a semester
select semester, avg(gpa) from student_result group by semester;

-- Q # 9:display all the student records, if the student email id is null it should be displayed as “no valid email address”.
...
-- Q # 10:Write a query which will display the student name, branch, registration number, semester number and result. Display the full name of EEE as well as ECE branch as mentioned below,If EEE then ‘Electrical and Electronic Engineering’
